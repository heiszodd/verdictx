...
def test_full_fulfillment_is_normalized_to_100_percent(direct_vm, direct_deploy, direct_alice, direct_bob):
    contract = direct_deploy("contracts/VerdictX.py", "VX-FULL", direct_bob, direct_bob)
    ...
    direct_vm.mock_llm(
        ...
        json.dumps({
            "decision": "FULL_FULFILLMENT",
            "fulfillment_score": 100,
            "valid_deliverables": 10,
            "invalid_deliverables": 0,
            "recommended_payment_percentage": 84,
            "confidence": 97,
            "findings": ["All required deliverables were evidenced."],
            "reasoning": "Every acceptance criterion was satisfied.",
        }),
    )
...
def test_non_fulfillment_is_normalized_to_zero_percent(direct_vm, direct_deploy, direct_alice, direct_bob):
    contract = direct_deploy("contracts/VerdictX.py", "VX-NONE", direct_bob, direct_bob)
    direct_vm.sender = direct_alice
    direct_vm.mock_llm(
        r".*impartial adjudicator.*",
        json.dumps({
            "decision": "NON_FULFILLMENT",
            "fulfillment_score": 0,
            "valid_deliverables": 0,
            "invalid_deliverables": 10,
            "recommended_payment_percentage": 73,
            "confidence": 94,
            "findings": [],
            "reasoning": "The provider did not satisfy the acceptance criteria.",
        }),
    )
...
def test_only_buyer_can_adjudicate(direct_vm, direct_deploy, direct_alice, direct_bob, direct_charlie):
    contract = direct_deploy("contracts/VerdictX.py", "VX-AUTH", direct_bob, direct_bob)
