...
def parse_model_output(value):
    ...
    try:
        ...
        try:
            parsed=json.loads(text[start:end+1]); return parsed if isinstance(parsed,dict) else None
        except Exception: return None

class VerdictX(gl.Contract):
    case_id:str; buyer:Address; provider:Address; escrow:Address; adjudicator:Address; decision:str
    fulfillment_score:u256; valid_deliverables:u256; invalid_deliverables:u256; recommended_payment_percentage:u256; confidence:u256
    reasoning:str; verdict:str; bridge_emitted:bool; settled:bool

    def __init__(self,case_id:str,provider:Address,escrow:Address):
        self.case_id=case_id; self.buyer=gl.message.sender_address; self.provider=provider; self.escrow=escrow; self.adjudicator=gl.message.sender_address
        self.decision="PENDING"; self.fulfillment_score=u256(0); self.valid_deliverables=u256(0); self.invalid_deliverables=u256(0); self.recommended_payment_percentage=u256(0); self.confidence=u256(0)
        self.reasoning=""; self.verdict="PENDING"; self.bridge_emitted=False; self.settled=False
    ...
    def adjudicate(self,agreement:str,delivery:str,dispute:str,evidence_urls:list[str])->str:
        ...
        def evaluate():
            ...
            for url in evidence_urls:
                try:
                    page=gl.nondet.web.render(url,mode="text"); material+=f"\nSOURCE {url}\n{str(page)[:MAX_TEXT_LENGTH]}"
        ...
        def validate(leader_result):
            ...
            try:
                ...
                d=str(p.get("decision","")); di=str(i.get("decision","")); ps=int(p.get("fulfillment_score",-1)); ins=int(i.get("fulfillment_score",-1)); pp=int(p.get("recommended_payment_percentage",-1)); ip=int(i.get("recommended_payment_percentage",-1)); pv=int(p.get("valid_deliverables",-1)); iv=int(i.get("valid_deliverables",-1)); pi=int(p.get("invalid_deliverables",-1)); ii=int(i.get("invalid_deliverables",-1))
                return d in ALLOWED_DECISIONS and di in ALLOWED_DECISIONS and d==di and 0<=ps<=100 and 0<=ins<=100 and 0<=pp<=100 and 0<=ip<=100 and 0<=pv<=MAX_DELIVERABLE_COUNT and 0<=iv<=MAX_DELIVERABLE_COUNT and 0<=pi<=MAX_DELIVERABLE_COUNT and 0<=ii<=MAX_DELIVERABLE_COUNT and abs(ps-ins)<=10 and abs(pp-ip)<=10 and abs(pv-iv)<=1 and abs(pi-ii)<=1
