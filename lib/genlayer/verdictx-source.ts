export const VERDICTX_CONTRACT_SOURCE = `# { "Depends": "py-genlayer:9b8kjyda2ycxyq4ea6g4yfpnydxhd52gqba5rb8dw7krkh5mn9p0" }
...
class VerdictX(gl.Contract):
    ...
    def __init__(self, case_id: str, provider: Address):
        self.case_id=case_id; self.buyer=gl.message.sender_address; self.provider=provider; self.adjudicator=gl.message.sender_address
        self.amount=u256(0); self.funded=False; self.decision="PENDING"; self.fulfillment_score=u256(0); self.valid_deliverables=u256(0); self.invalid_deliverables=u256(0); self.recommended_payment_percentage=u256(0); self.confidence=u256(0); self.reasoning=""; self.verdict="PENDING"; self.settled=False
    ...
    def adjudicate(self, agreement:str, delivery:str, dispute:str, evidence_urls:list[str])->str:
        ...
        def evaluate()->dict:
            source_material=""
            for url in evidence_urls:
                try: page=gl.nondet.web.render(url, mode="text"); source_material+=f"\\nSOURCE {url}\\n{str(page)[:MAX_TEXT_LENGTH]}"
                except Exception: source_material+=f"\\nSOURCE {url}\\nUNAVAILABLE"
            prompt=f'''You are the impartial adjudicator for VerdictX. Evaluate the agreement, delivery, dispute, and evidence. Do not invent unavailable facts. Scores and payment are 0-100; counts are non-negative integers. Return ONLY JSON with decision, fulfillment_score, valid_deliverables, invalid_deliverables, recommended_payment_percentage, confidence, findings, reasoning. decision must be FULL_FULFILLMENT, PARTIAL_FULFILLMENT, NON_FULFILLMENT, INVALID_CASE, or INCONCLUSIVE.\\nAGREEMENT:\\n{agreement}\\nDELIVERY:\\n{delivery}\\nDISPUTE:\\n{dispute}\\nEVIDENCE:\\n{source_material}'''
            return _parse_model_output(gl.nondet.exec_prompt(prompt,response_format="json"))
        def validate(leader_result)->bool:
            if not isinstance(leader_result,gl.vm.Return) or not isinstance(leader_result.calldata,dict): return False
            try:
                p=leader_result.calldata; i=evaluate(); allowed={"FULL_FULFILLMENT","PARTIAL_FULFILLMENT","NON_FULFILLMENT","INVALID_CASE","INCONCLUSIVE"}
                d=str(p.get("decision","")); di=str(i.get("decision",""))
                ps=int(p.get("fulfillment_score",-1)); ins=int(i.get("fulfillment_score",-1)); pp=int(p.get("recommended_payment_percentage",-1)); ip=int(i.get("recommended_payment_percentage",-1)); pv=int(p.get("valid_deliverables",-1)); iv=int(i.get("valid_deliverables",-1)); pi=int(p.get("invalid_deliverables",-1)); ii=int(i.get("invalid_deliverables",-1))
        ...
        try: decision=str(result["decision"]); score=int(result["fulfillment_score"]); valid=int(result["valid_deliverables"]); invalid=int(result["invalid_deliverables"]); payment=int(result["recommended_payment_percentage"]); confidence=int(result["confidence"])
        except Exception: raise gl.UserError("Adjudication returned malformed structured data")
        if decision not in {"FULL_FULFILLMENT","PARTIAL_FULFILLMENT","NON_FULFILLMENT","INVALID_CASE","INCONCLUSIVE"} or not 0<=score<=100 or not 0<=payment<=100 or not 0<=confidence<=100 or not 0<=valid<=MAX_DELIVERABLE_COUNT or not 0<=invalid<=MAX_DELIVERABLE_COUNT: raise gl.UserError("Invalid adjudication result values")
        self.decision=decision; self.fulfillment_score=u256(score); self.valid_deliverables=u256(valid); self.invalid_deliverables=u256(invalid); self.recommended_payment_percentage=u256(payment); self.confidence=u256(confidence); self.reasoning=str(result.get("reasoning","")); self.verdict=json.dumps(result,sort_keys=True); return self.verdict
    ...
    def settle(self):
        ...
        provider_amount=(self.amount*self.recommended_payment_percentage)//u256(100); refund=self.amount-provider_amount; self.settled=True
