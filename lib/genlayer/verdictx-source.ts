...
{{
  ...
  "reasoning": "..."
        ...
        def validate(leader_result) -> bool:
            ...
            try:
                ...
                if not 0 <= proposed_confidence <= 100:
                    return False
                if proposed_valid < 0 or proposed_invalid < 0 or independent_valid < 0 or independent_invalid < 0:
                    return False
                return decision == independent_decision and abs(proposed_score - independent_score) <= 10 and abs(proposed_payment - independent_payment) <= 10 and proposed_valid == independent_valid and proposed_invalid == independent_invalid
            except Exception:
                return False
        ...
        if decision not in {"FULL_FULFILLMENT", "PARTIAL_FULFILLMENT", "NON_FULFILLMENT", "INVALID_CASE", "INCONCLUSIVE"}:
            raise gl.UserError("Invalid adjudication decision")
        if not 0 <= score <= 100 or not 0 <= payment <= 100 or not 0 <= confidence <= 100:
