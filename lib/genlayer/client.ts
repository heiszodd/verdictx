...
function readString(value: unknown, ...keys: string[]): string | undefined {
  ...
  for (const key of keys) { const candidate = record?.[key]; if (typeof candidate === 'string' && candidate.trim()) return candidate.trim(); }
  return undefined;
}
function readBoolean(value: unknown, ...keys: string[]): boolean | undefined {
  const record = asRecord(value);
  for (const key of keys) { const candidate = record?.[key]; if (typeof candidate === 'boolean') return candidate; }
  return undefined;
}
function readNumber(value: unknown, ...keys: string[]): number | undefined {
  const record = asRecord(value);
  for (const key of keys) { const candidate = record?.[key]; const number = typeof candidate === 'number' ? candidate : typeof candidate === 'string' && candidate.trim() ? Number(candidate) : NaN; if (Number.isFinite(number)) return number; }
  return undefined;
}
function requireAddress(value: unknown, label = 'Contract address'): `0x${string}` {
  if (typeof value !== 'string' || !ADDRESS_RE.test(value)) throw new Error(`${label} is missing or invalid. Expected a 20-byte EVM address.`);
...
export function getGenLayerClient(account?: ClientAccount, provider?: Eip1193Provider): GenLayerClient {
  return createClient({ chain: studioDevnet, ...(account ? { account } : {}), ...(provider ? { provider } : {}) } as Parameters<typeof createClient>[0]);
}
...
async function ensureGenLayerNetwork(provider: Eip1193Provider): Promise<void> {
  ...
  if (currentChainId !== GENLAYER_CHAIN_ID_HEX) {
    try {
      ...
      await provider.request({ method: 'wallet_addEthereumChain', params: [{ chainId: GENLAYER_CHAIN_ID_HEX, chainName: 'GenLayer Studio Devnet', nativeCurrency: { name: 'GEN', symbol: 'GEN', decimals: 18 }, rpcUrls: [GENLAYER_RPC], blockExplorerUrls: [GENLAYER_EXPLORER] }] });
    }
  }
...
function findAddress(value: unknown): `0x${string}` | undefined {
  if (typeof value === 'string') {
    if (ADDRESS_RE.test(value)) return value as `0x${string}`;
    if (/^0x[0-9a-fA-F]{64}$/.test(value)) {
      const candidate = `0x${value.slice(-40)}`;
      if (ADDRESS_RE.test(candidate) && candidate !== '0x0000000000000000000000000000000000000000') return candidate as `0x${string}`;
    }
    return undefined;
  }
  if (!value || typeof value !== 'object') return undefined;
  const record = value as Record<string, unknown>;
  for (const key of ['contractAddress','contract_address','deployedAddress','deployed_address','address']) {
    const found = findAddress(record[key]);
    if (found) return found;
  }
  for (const key of ['returnData','return_data','txExecutionResult','tx_execution_result','result']) {
    const found = findAddress(record[key]);
    if (found) return found;
  }
  return undefined;
...
async function deployedAddress(client: GenLayerClient, hash: VerdictXTransaction): Promise<`0x${string}`> {
  const receipt = await client.waitForTransactionReceipt({ hash: hash as never, status: TransactionStatus.FINALIZED, fullTransaction: true });
  if (!transactionSucceeded(receipt)) throw new Error(`Deployment failed: ${statusName(receipt)} / ${executionName(receipt)}`);
  const direct = findAddress(receipt);
  if (direct) return direct;
  const tx = await client.getTransaction({ hash: hash as never });
  const fromTx = findAddress(tx);
...
export async function settleEscrow(account: ClientAccount, contractAddress: `0x${string}`, provider: Eip1193Provider, adjudicationHash: VerdictXTransaction): Promise<VerdictXTransaction> {
  ...
  if (adjudication.status !== 'FINALIZED' || adjudication.execution !== 'FINISHED_WITH_RETURN') throw new Error('Settlement is locked until the GenLayer adjudication transaction is FINALIZED and successful.');
...
export async function getEscrowState(contractAddress: `0x${string}`) {
  ...
  const names = ['get_amount','is_funded','is_verdict_finalized','is_settlement_blocked','is_settled','get_decision','get_payment_percentage','get_provider_amount','get_buyer_refund','get_verdict_contract','get_case'];
  const [amount,funded,verdictFinalized,blocked,settled,decision,paymentPercentage,providerAmount,buyerRefund,verdictContract,caseId] = await Promise.all(names.map((functionName) => client.readContract({ address, functionName, args: [] })));
...
export async function getAdjudicationTransaction(hash: VerdictXTransaction): Promise<AdjudicationStatus> {
  ...
  if (status === 'PENDING' && queuePosition === null) { try { const number = Number(await client.getTransactionQueuePosition({ hash: hash as never })); queuePosition = Number.isFinite(number) ? number : null; } catch { queuePosition = null; } }
  return { hash, status, execution: executionName(transaction), lifecycle, projectedStatus: (readString(projection,'projectedStatus','projected_status') ?? status).toUpperCase(), resolutionAction: readString(projection,'resolutionAction','resolution_action'), resolutionSource: readString(projection,'resolutionSource','resolution_source'), decisionActive: readBoolean(projection,'decisionActive','decision_active'), queuePosition, recipient: readString(record,'recipient'), error: extractExecutionError(transaction), executionHash: readString(record,'txExecutionHash','tx_execution_hash'), timestamps: asRecord(record?.timestamps) ?? undefined, raw: transaction };
}
