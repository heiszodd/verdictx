...
export function getGenLayerClient(account?: ClientAccount, provider?: unknown) {
  return createClient({
...
export async function submitAdjudication(account: ClientAccount, agreement: string, delivery: string, dispute: string, evidenceUrls: string[]): Promise<VerdictXTransaction> {
  const client = getGenLayerClient(account, typeof window !== 'undefined' ? window.ethereum : undefined);
