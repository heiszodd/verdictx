...
export function getGenLayerClient(account?: ClientAccount, provider?: Eip1193Provider) { return createClient({ chain: testnetBradbury, account: account, provider: provider as any } as any); }
...
export async function deployVerdictXCase(account: ClientAccount, caseId: string, providerAddress: `0x${string}`, provider: Eip1193Provider): Promise<{ hash: VerdictXTransaction; address: `0x${string}` }> { const walletAccount = asGenLayerAccount(account); const normalizedProvider = normalizeAddress(providerAddress); await ensureBradbury(provider); const client = getGenLayerClient(account, provider); const hash = await client.deployContract({ account: walletAccount, code: VERDICTX_CONTRACT_SOURCE.trimStart(), args: [caseId, normalizedProvider] }); const receipt = await client.waitForTransactionReceipt({ hash: hash as any, status: TransactionStatus.ACCEPTED }); const rawReceipt = receipt as any; let transaction: any = null; try { transaction = await client.getTransaction({ hash: hash as any }); } catch {} const candidate = rawReceipt?.txDataDecoded?.contractAddress ?? rawReceipt?.contractAddress ?? rawReceipt?.contract_address ?? transaction?.txDataDecoded?.contractAddress ?? transaction?.recipient ?? transaction?.contractAddress ?? transaction?.contract_address; return { hash: hash as VerdictXTransaction, address: requireAddress(candidate, 'Deployed contract address') }; }
...
function extractExecutionError(transaction: any): string | undefined {
  const candidates = [transaction?.error, transaction?.executionError, transaction?.txExecutionError, transaction?.txExecutionResultMessage, transaction?.resultMessage, transaction?.errorMessage, transaction?.txDataDecoded?.error, transaction?.receipt?.error];
  for (const value of candidates) if (typeof value === 'string' && value.trim()) return value.trim();
  return undefined;
}
async function getLifecycleProjection(client: any, hash: VerdictXTransaction): Promise<any | null> {
  try { return await client.request({ method: 'gen_getTransactionLifecycle', params: [{ txId: hash }] }); } catch { return null; }
}
