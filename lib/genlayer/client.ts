...
const GENLAYER_NETWORK = { chainId: GENLAYER_CHAIN_ID_HEX, chainName: 'GenLayer Studio Next', nativeCurrency: { name: 'GEN', symbol: 'GEN', decimals: 18 }, rpcUrls: [GENLAYER_RPC], blockExplorerUrls: [GENLAYER_EXPLORER] } as const;
const genlayerChain = {
  ...testnetBradbury,
  id: GENLAYER_CHAIN_ID,
  rpcUrls: { ...testnetBradbury.rpcUrls, default: { http: [GENLAYER_RPC] }, public: { http: [GENLAYER_RPC] } },
  blockExplorers: { default: { name: 'GenLayer Explorer', url: GENLAYER_EXPLORER } },
};
...
function readNumber(value: unknown, ...keys: string[]): number | undefined { const record = asRecord(value); for (const key of keys) { const candidate = record?.[key]; const n = typeof candidate === 'number' ? candidate : typeof candidate === 'string' && candidate.trim() ? Number(candidate) : NaN; if (Number.isFinite(n)) return n; } return undefined; }
...
function asGenLayerAccount(account: ClientAccount) { return { address: requireAddress(account, 'Account address'), type: 'json-rpc' as const }; }

export function getGenLayerClient(account?: ClientAccount, provider?: Eip1193Provider): GenLayerClient { return createClient({ chain: genlayerChain, ...(account ? { account } : {}), ...(provider ? { provider } : {}) } as Parameters<typeof createClient>[0]); }
...
async function ensureGenLayerNetwork(provider: Eip1193Provider): Promise<void> {
  ...
  catch (error: unknown) { const code = readNumber(error, 'code'); if (code !== 4902) throw error; await provider.request({ method: 'wallet_addEthereumChain', params: [GENLAYER_NETWORK] }); }
}
...
export async function deployVerdictXCase(account: ClientAccount, caseId: string, providerAddress: `0x${string}`, provider: Eip1193Provider): Promise<{ hash: VerdictXTransaction; address: `0x${string}`; escrowAddress: `0x${string}`; escrowHash: VerdictXTransaction; configureHash: VerdictXTransaction }> {
  const walletAccount=asGenLayerAccount(account); const buyer=normalizeAddress(account); const providerAddr=normalizeAddress(providerAddress); await ensureGenLayerNetwork(provider); const client=getGenLayerClient(account,provider);
  const escrowHash=await client.deployContract({account:walletAccount,code:ESCROW_CONTRACT_SOURCE.trimStart(),args:[caseId,buyer,providerAddr]}) as VerdictXTransaction;
...
export async function settleEscrow(account: ClientAccount, contractAddress: `0x${string}`, provider: Eip1193Provider, adjudicationHash: VerdictXTransaction): Promise<VerdictXTransaction> {
  const walletAccount=asGenLayerAccount(account); const address=requireAddress(contractAddress); await ensureGenLayerNetwork(provider);
  const adjudication=await getAdjudicationTransaction(adjudicationHash); if(adjudication.status!=='FINALIZED'||adjudication.execution!=='FINISHED_WITH_RETURN') throw new Error('Settlement is locked until the GenLayer adjudication transaction is FINALIZED and successful.');
  const escrow=await getEscrowState(address); if(!escrow.verdictFinalized) throw new Error('The finalized verdict has not reached the escrow settlement bridge yet. Wait for the bridge transaction.'); if(escrow.settlementBlocked) throw new Error('This case is INVALID or INCONCLUSIVE; escrow remains locked.');
  const client=getGenLayerClient(account,provider); return await client.writeContract({account:walletAccount,address,functionName:'settle',args:[],value:0n}) as VerdictXTransaction;
}

export async function getEscrowState(contractAddress: `0x${string}`) {
  const address=requireAddress(contractAddress); const client=getGenLayerClient();
  const [amount,funded,verdictFinalized,blocked,settled,decision,paymentPercentage,providerAmount,buyerRefund,verdictContract,caseId]=await Promise.all([
    client.readContract({address,functionName:'get_amount',args:[]}),client.readContract({address,functionName:'is_funded',args:[]}),client.readContract({address,functionName:'is_verdict_finalized',args:[]}),client.readContract({address,functionName:'is_settlement_blocked',args:[]}),client.readContract({address,functionName:'is_settled',args:[]}),client.readContract({address,functionName:'get_decision',args:[]}),client.readContract({address,functionName:'get_payment_percentage',args:[]}),client.readContract({address,functionName:'get_provider_amount',args:[]}),client.readContract({address,functionName:'get_buyer_refund',args:[]}),client.readContract({address,functionName:'get_verdict_contract',args:[]}),client.readContract({address,functionName:'get_case',args:[]}),
  ]);
