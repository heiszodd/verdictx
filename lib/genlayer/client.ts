...
async function ensureBradbury(provider: Eip1193Provider): Promise<void> {
  ...
  catch (error: unknown) { const code = readNumber(error, 'code'); if (code !== 4902) throw error; await provider.request({ method: 'wallet_addEthereumChain', params: [BRADBURY_NETWORK] }); }
}
...
export async function deployVerdictXCase(account: ClientAccount, caseId: string, providerAddress: `0x${string}`, provider: Eip1193Provider): Promise<{ hash: VerdictXTransaction; address: `0x${string}`; escrowAddress: `0x${string}`; escrowHash: VerdictXTransaction; configureHash: VerdictXTransaction }> {
  const walletAccount=asGenLayerAccount(account); const buyer=normalizeAddress(account); const providerAddr=normalizeAddress(providerAddress); await ensureBradbury(provider); const client=getGenLayerClient(account,provider);
...
export async function settleEscrow(account: ClientAccount, contractAddress: `0x${string}`, provider: Eip1193Provider, adjudicationHash: VerdictXTransaction): Promise<VerdictXTransaction> {
  const walletAccount=asGenLayerAccount(account); const address=requireAddress(contractAddress); await ensureBradbury(provider);
