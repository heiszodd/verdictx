...
class VerdictX(gl.Contract):
    ...
    @gl.public.view
    def is_bridge_emitted(self) -> bool:
        return self.bridge_emitted
    ...
    def adjudicate(
        ...
        if max(len(agreement), len(delivery), len(dispute)) > MAX_TEXT_LENGTH:
        ...
        def validate(leader_result):
            if not isinstance(leader_result, gl.vm.Return) or not isinstance(leader_result.calldata, dict):
                return False
            try:
                p = leader_result.calldata
                i = evaluate()
                if not isinstance(i, dict):
                    return False
                d = str(p.get("decision", ""))
                di = str(i.get("decision", ""))
                ps = int(p.get("fulfillment_score", -1))
                ins = int(i.get("fulfillment_score", -1))
                pp = int(p.get("recommended_payment_percentage", -1))
                ip = int(i.get("recommended_payment_percentage", -1))
                pv = int(p.get("valid_deliverables", -1))
                iv = int(i.get("valid_deliverables", -1))
                pi = int(p.get("invalid_deliverables", -1))
                ii = int(i.get("invalid_deliverables", -1))
                return (
                    d in ALLOWED_DECISIONS
                    and di in ALLOWED_DECISIONS
                    and d == di
                    and 0 <= ps <= 100
                    and 0 <= ins <= 100
                    and 0 <= pp <= 100
                    and 0 <= ip <= 100
                    and 0 <= pv <= MAX_DELIVERABLE_COUNT
                    and 0 <= iv <= MAX_DELIVERABLE_COUNT
                    and 0 <= pi <= MAX_DELIVERABLE_COUNT
                    and 0 <= ii <= MAX_DELIVERABLE_COUNT
                    and abs(ps - ins) <= 10
                    and abs(pp - ip) <= 10
                    and abs(pv - iv) <= 1
                    and abs(pi - ii) <= 1
        ...
        if (
            ...
            or not 0 <= score <= 100
            or not 0 <= payment <= 100
            or not 0 <= confidence <= 100
            or not 0 <= valid <= MAX_DELIVERABLE_COUNT
            or not 0 <= invalid <= MAX_DELIVERABLE_COUNT
        ):
            raise gl.UserError("invalid adjudication result values")

        if decision == "FULL_FULFILLMENT":
            payment = 100
        elif decision == "NON_FULFILLMENT":
            payment = 0
        ...
        result["recommended_payment_percentage"] = payment
        self.verdict = json.dumps(result, sort_keys=True)

        # Critical bridge invariant: this message is created with on='finalized'.
        # The escrow cannot authorize settlement until this child message executes.
        EscrowBridge(self.escrow).emit(on="finalized").apply_verdict(
            self.case_id,
    ...
    def mark_settled(self):
        if gl.message.sender_address != self.escrow:
            raise gl.UserError("only the configured escrow can mark the case settled")
        if self.decision == "PENDING" or not self.bridge_emitted:
            raise gl.UserError("case has not reached the settlement bridge")
        self.settled = True
